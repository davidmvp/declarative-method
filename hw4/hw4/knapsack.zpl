...
param radioactivity[I] := read "knapsack.txt" as "<1s> 4n"; 
...
var total;
var rado;
maximize total:  (sum <s> in I: take[s] * value[s])   ; 
subto maxweight:     sum <w> in I: take[w] * weight[w] <= maxweight;
subto spareweight:  maxweight - (sum <w> in I:   take[w] * weight[w]) == spareweight;
subto count:  sum<s> in I: take[s] == count;
subto rado: sum<s> in I: take[s] * radioactivity[s] <= 20;
subto totalvalue:  sum<s> in I: take[s] * value[s] == totalvalue;
